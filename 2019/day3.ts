...
interface WireGridPosition {
  x: number;
  y: number;
}

const INITIAL_POSITION: WireGridPosition = { x: 0, y: 0 };
...
const getPositionDistance = (posA: WireGridPosition, posB: WireGridPosition) => {
  return Math.abs((posA.x - posB.x)) + Math.abs((posA.y - posB.y));
}

const findClosestIntersection = (rawA, rawB) => {
  //x, {[y]: steps}
  const wireA: Record<number, Record<number, number>> = {};
  const wireB = {};
  ...
  rawA.forEach(instr => {
    ...
    let distance = Number(instr.slice(1));
    ...
    while(distance > 0) {
      ...
      steps++;
      distance--;
      const yCoordinatesAtX = wireA[position.x] || {};
      yCoordinatesAtX[position.y] = yCoordinatesAtX[position.y] || steps;
      wireA[position.x] = yCoordinatesAtX;
    }
  });
  ...
  rawB.forEach(instr => {
    ...
    while(distance > 0) {
      ...
      distance--;
      const yCoordinatesAtX = wireA[position.x] || {};
      if(yCoordinatesAtX[position.y]) {
        minSteps = Math.min(minSteps, yCoordinatesAtX[position.y] + steps);
      };
    }
  })
